...
K=1;                      %repeating time 
% t=cputime;
MeanAccuracy = zeros(1,K);%%% updated 
AverageTime = zeros(1,K);%%% updated 
...
% 	    	u = cvset;
% 	        D1 = cvlabel;    
	        f=zeros(size(testset,1),1);
            tem22=zeros(K_cv); % error number
	        for j=1:size(testlabel,1)
	            f(j)=alpha(n1+1); 
	            for i=1:n1
	            f(j)=f(j)+alpha(i)*Y(i,i)*gausskernel(trainset(i,:),testset(j,:), p);
	            end
	            f(j)=testlabel(j)*f(j);
	        end
	        ...
	        for i=1:size(testset,1)
	            if f(i)<0
	                tem22(K_cv)=tem22(K_cv)+1;%error number
                end       
            end	  
             m1 = ( 1-(tem22)/length(testlabel) )* 100;
	         s=std(tem22)/length(testlabel) * 100;
