...
%----------------------------------above----------------------------------------	    	    
      ...
      % get the best parameters 	   	    
        ...
        % logp(ipmax)
        % im
    % then apply the best parameter to test the training set

    tic;%---------------------------------------------------------------
    % test the obtained classifier on training dataset
    C = 2 ^ logC(iCmax);
...
mAccuracy = mean(1 - mean(errAcc)/size(u,1))
stdAcc = mean(std(errAcc/size(u, 1)))
mTime = mean(AverageTime)
maxAcc =mean(1- min((errAcc)/size(u, 1)))
minAcc =mean(1- max((errAcc)/size(u, 1)))
